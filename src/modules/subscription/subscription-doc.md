...
- `adapters/billing/GoogleReceiptAdapter.ts` Google Play Developer API'ye (subscriptionsv2.get) baglanir. `acknowledgementState === ACKNOWLEDGEMENT_STATE_PENDING` ve purchase aktifse server-side `purchases.subscriptions.acknowledge` (v1) cagrilir — best-effort, verification'i BLOKLAMAZ (client `completePurchase()` + RTDN reconcile yedek). Response'ta `linkedPurchaseToken` varsa (plan degisikligi/upgrade-downgrade) `validate()` bunu doner; `productId` opsiyoneldir (RTDN'nin daha once hic gormedigi bir token'i cross-check'siz sorgulayabilmesi icin — asagi bkz). `AppleReceiptAdapter.ts` henuz stub (Android-only, bkz kontrat). `adapters/repository/PrismaSubscriptionRepository.ts` DB yazimini yapar; `supersede()` linked eski satiri `status: 'superseded'` yapar (best-effort, hata atmaz).
...
- **Plan degisikligi (`linkedPurchaseToken` zinciri)**: Play, uygulama disinda (Play Store > Abonelikler) plan degistirmeye izin verir — bu yeni bir `purchaseToken` uretir ve Google'in cevabinda eskisine isaret eden `linkedPurchaseToken` olur. `PurchaseSubscription` (client `/verify` cagirdiginda) ve `processPlayRtdnJob` (RTDN'den, hic bilinmeyen bir token icin bile — `linkedPurchaseToken` zincirini takip ederek dogru kullaniciyi bulur) bu alani gorunce eski satiri `supersede()` ile kapatir. `findLatestByUserId` artik salt `updatedAt`'e degil, once entitled (active/trialing + suresi gecmemis) satirlara bakar — yoksa supersede edilen eski satir sonradan tekrar dokunulunca "en son" sanilip yanlis plani gosterebilirdi.
