...
**Postgres testcontainer'ları `pgvector/pgvector:pg16` image'ını kullanmalı**, düz
`postgres:16-alpine` DEĞİL — schema.prisma'nın migration'ı pgvector extension'ı
gerektiriyor, düz image'da migration patlar (identity/food-recognition turlarında
keşfedilen ve artık shared-rule.md'ye eklenen kalıcı bir kural).

**ÖNEMLİ — repoda bu modüle ait `test.todo` stub'ları olabilir** (genel iskelet
kurulumu turunda, iş mantığı yazılmadan önce boş test placeholder'ları oluşturulmuştu).
Kod yazmaya başlamadan önce `src/modules/onboarding-plan/` altında `test.todo` içeren
dosya var mı diye TARA. Varsa, onları SİLİP yeniden yazmak yerine, doğru testlerle
DOLDUR (böylece hangi davranışın baştan planlandığı kaybolmaz).
