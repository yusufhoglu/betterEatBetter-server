...
## `UpdatePlan` — `goal-management` için public yazma use-case'i (SONRADAN EKLENDİ)

- `goal-management` modülünün `UpdateGoal`'ı, `UserProfile`/`Plan` tablolarına ASLA
  kendi Prisma sorgusuyla yazamaz (tablolar bu modülün sahipliğinde). Bunun yerine bu
  modül `use-cases/UpdatePlan.ts` adında bir public YAZMA use-case'i sunar:
  `execute(userId, changes: { weightKg?, workoutsPerWeek?, goal?, weeklyPaceKg? }): Promise<Plan>`
- Akış: mevcut `UserProfile` yüklenir → YOKSA `NotFoundError('NOT_ONBOARDED')` →
  verilen alanlar profile merge edilip kaydedilir → `PlanCalculationService.computePlan`
  TAM profil ile (değişmeyen `heightCm`/`age`/`gender` profildekilerle) yeniden
  çağrılır → mevcut `Plan` satırı GÜNCELLENIR (yeni satır açılmaz, `updatedAt` değişir).
- `heightCm`/`age`/`gender` bu use-case üzerinden DEĞİŞTİRİLEMEZ — bunlar profil
  düzenleme kapsamı (ileride ayrı ele alınır), hedef güncelleme kapsamı değil.
